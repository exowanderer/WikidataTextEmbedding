...
def extract_specific_word_embedding(model, tokenizer, sentence, start_marker='<w>', end_marker='</w>'):
    ...
    if start_marker in sentence and end_marker in sentence:
        ...
        target_word = sentence[start_index + len(start_marker):end_index - len(end_marker)]
        after_target = sentence[end_index:]

        # Clean the sentence by removing the markers
        tokens_before = tokenizer(before_target, add_special_tokens=False)['input_ids']
        tokens_target = tokenizer(target_word, add_special_tokens=False)['input_ids']

        # Reconstruct the input with markers removed and tokenize
        clean_sentence = before_target + target_word + after_target
        inputs = tokenizer(clean_sentence, return_tensors="pt").to(model.device)
        ...
        adapter_mask = torch.full((1,), task_id, dtype=torch.int32).to(model.device)
        with torch.no_grad():
        ...
        normalized_embeddings = outputs[0][:, target_start_pos:target_end_pos].mean(dim=0)
...
# Example usage
sentence = "I went to the river bank to take a swim. Afterwards, I went to the <w>bank</w> to withdraw some money."
embedding = extract_specific_word_embedding(model.model, model.tokenizer, sentence)
